...
class simple_simulation(Job):
    ...
    def run(self,
        ...
        for seed_val in np.arange(n_seeds):
            ...
            sim_feats = ['sim_exp', 'sim_knee_freq', 'sim_freq', 'sim_bw', 
            ...
            if kind == 'exponent_delta':
                ...
                exponent2 = -2
                sig = sim_knee(n_seconds=n_secs, 
                                fs=fs, exponent1=0, 
                                exponent2=exponent2, knee=knee_freq ** np.abs(exponent2))
                ...
                sig = sim_knee(n_seconds=n_secs, 
                                fs=fs, exponent1=0, 
                                exponent2=exp, knee=knee_freq ** np.abs(exp))
            ...
            data = {'specparam': {'periodic': pd.concat(p_sp_l_f),
                                  'aperiodic': pd.concat(ap_sp_l_f),
                                },
                    'pyrasa': {'periodic': pd.concat(p_ir_l),
