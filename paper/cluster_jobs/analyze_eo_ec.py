...
class eo_ec_analysis(Job):
    ...
    def run(self,
        ...
        if subject_id < 10:
            cur_file = list(Path(base_p + '/data/SPIS-Resting-State-Dataset-master/Pre-SART EEG').glob(f'S0{subject_id}*_{condition}.mat'))[0]
        else:
            cur_file = list(Path(base_p + '/data/SPIS-Resting-State-Dataset-master/Pre-SART EEG').glob(f'S{subject_id}*_{condition}.mat'))[0]

        sig = read_mat(cur_file)['dataRest']
        fs = 256

        psd_kwargs = {'nperseg': fs*4, 
                      'noverlap': fs*2, 
                      'average': 'median'}
        ...
        data = {'periodics': {'peak_params': peak_params,
                            'alpha': alpha_df},
                'aperiodics': {'fixed': fixed,
                            'knee': knee},
                'psd': {'psd': psd,
                        'freq': freq}}
