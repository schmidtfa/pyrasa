...
class super_simulation(Job):
    ...
    exclude_kwargs_from_filename = ['n_secs', 'exp', 'knee', 'freq',
                                    'bw', 'height', 'b_proba']
    ...
    def run(self,
        ...
        kind = subject_id
        burst_list, p_ir_l, ap_ir_l_k, ap_ir_l_f,  = [], [], [], []
        ...
        for seed_val in np.arange(n_seeds):
            ...
            sim_feats = ['sim_exp', 'sim_knee', 'sim_freq', 'sim_bw', 
                        'sim_height', 'sim_b_proba', 'seed', 'kind']
            sim_params = dict(zip(sim_feats, np.zeros(len(sim_feats))*np.nan))

            if kind == 'fixed':
                comps_osc = {'sim_powerlaw' : {'exponent' : exp},
                ...
                sig_ap = sim_knee(n_seconds=n_secs, 
                                fs=fs, exponent1=exp1, 
                                exponent2=exp, knee=knee)
                ...
                sig_bursty = sim_bursty_oscillation(n_seconds=n_secs, 
                                                    fs=fs, freq=freq,
                                                    burst_params={'enter_burst': b_proba,
                                                                  'leave_burst': b_proba})
                ...
                _, periodic, freqs, times = irasa_sprint(sig[np.newaxis,:],
                                                        fs=fs,
                                                        band=(1, 100),
                                                        smooth=False,
                                                        n_avgs=[3, 5, 7]
                                                        )
                df_peaks = get_peak_params_sprint(periodic, 
                                                freqs=freqs, 
                                                times=times,
                ...
                burst_detection = {'percent_cycles_detected' : np.mean(detected),
            ...
            # do irasa testing
            freqs, aperiodic, periodic = irasa(sig, fs=fs, 
                                               band=(1,100), 
            ...
            df_p = get_peak_params(periodic, freqs, 
                                min_peak_height=min_peak_height,
            ...
            fm_f = FOOOF(peak_width_limits=(1,8),
                        max_n_peaks=8,
                        min_peak_height=min_peak_height,
                        aperiodic_mode='fixed',
                        )
            fm_k = FOOOF(peak_width_limits=(1,8),
                        max_n_peaks=8,
                        min_peak_height=min_peak_height,
                        aperiodic_mode='knee',
                        )
            ...
            data = {'specparam': {'fixed': {'periodic': pd.concat(p_sp_l_f),
                                            'aperiodic': pd.concat(ap_sp_l_f),},
                                'knee': {'periodic': pd.concat(p_sp_l_k),
                                        'aperiodic': pd.concat(ap_sp_l_k),},
                                },
                    'pyrasa': {'fixed': {'periodic': pd.concat(p_ir_l),
                                        'aperiodic': pd.concat(ap_ir_l_f),},
                                'knee': {'periodic': pd.concat(p_ir_l),
    ...
    def extract_specparams(self, freqs, psd, fm, fit_type, df_sim):
        ...
        #i.e. 2*fmax, fmin/2
        fm.fit(freqs, psd, freq_range=(.5, 200)) 
        #aperiodics
        if fit_type == 'fixed':
            df_ap = pd.DataFrame({'Offset' : fm.get_params('aperiodic_params')[0],
                                'Exponent' : fm.get_params('aperiodic_params')[1],}, index=[0])
        
        elif fit_type == 'knee':
            df_ap = pd.DataFrame({'Offset' : fm.get_params('aperiodic_params')[0],
                                'Knee': fm.get_params('aperiodic_params')[1],
                                'Exponent_2' : fm.get_params('aperiodic_params')[2],}, index=[0])
            
            df_ap['Knee Frequency (Hz)'] = df_ap['Knee'] ** (1. / df_ap['Exponent_2'])
        ...
        #                          'bw': peak_params[2],}, index=[0])
            ...
            df_p = pd.DataFrame(peak_params, columns=('cf', 'pw', 'bw'), )
