...
# %%
sns.catplot(df_cmb, x='n_seconds', y='Error (Knee Frequency (Hz))', 
...
# %%
cor_cols = ['Offset', 'Exponent_1', 'Exponent_2', 'Knee Frequency (Hz)',  'relative Error (Knee Frequency (Hz))', 'relative Error (Exponent)']
...
for sec in [4, 8, 30, 60, 180, 300]:
    ...
    for col in cor_cols:
        cur_df = pd.DataFrame({'r': stats.pearsonr(irasa[col], no_rasa[col])[0],
                               'col': col,
                               'nsecs': sec}, index=[0])
...
grid = sns.FacetGrid(df_corrs, col='col', hue='col', palette='deep',
              col_wrap=3, height=4, aspect=2)
...
# %%
cols_of_interest = ['param_type', 'Knee', 'Offset', 'Exponent_2', 'Error (Knee Frequency (Hz))', 
                    'Error (Exponent)', 'Knee Frequency (Hz)']
...
sns.regplot(df_cmb_cut.query('param_type == "irasa"'), x='Error (Exponent)',
                 y='Knee Frequency (Hz)', 
                 #hue='param_type',
                 ax=axes[0])

axes[1].set_title('No IRASA')
sns.regplot(df_cmb_cut.query('param_type == "norasa"'), x='Error (Exponent)',
                 y='Knee Frequency (Hz)', 
                 #hue='param_type',
                 ax=axes[1])
# %%
...
axes[0].set_title('IRASA')
sns.regplot(df_cmb_cut.query('param_type == "irasa"'), x='Error (Knee Frequency (Hz))',
                 y='Knee Frequency (Hz)', 
                 #hue='param_type',
                 ax=axes[0])
axes[1].set_title('No IRASA')
sns.regplot(df_cmb_cut.query('param_type == "norasa"'), x='Error (Knee Frequency (Hz))',
                 y='Knee Frequency (Hz)', 
                 #hue='param_type',
                 ax=axes[1])
# %%
