...
def get_simulations(folder):
    ...
    for f in all_files:
        ...
        cur_p = pd.concat([cur_f['specparam']['periodic'],
                        cur_f['pyrasa']['periodic']])#.dropna(axis=1)


        cur_ap = pd.concat([cur_f['specparam']['aperiodic'],
                            cur_f['pyrasa']['aperiodic']
                        ])#.dropna(axis=1)
...
for exp in exp_list:
    ...
    df = pd.DataFrame({'Frequency (Hz)': f,
                        'Power': psd})
...
sns.lineplot(df_1.query('`Frequency (Hz)` < 100'), legend=False,
             x='Frequency (Hz)', y='Power', hue='exponent', palette=pal, ax=ax[0])
ax[0].set_xscale('log')
ax[0].set_yscale('log')

ax[1].scatter(df_ap.query('method == "pyrasa"')['Exponent'],
...
r, p =sts.pearsonr(df_ap.query('method == "pyrasa"')['Exponent'],
             df_ap.query('method == "specparam"')['Exponent'],)
r, p = np.round(r,2), np.round(p, 3)
ax[1].annotate(f'r = {r}', xy=(1.5, .6))
ax[1].annotate(f'p = {p}', xy=(1.5, .2))
...
sns.barplot(osc_per_run,
             x='method', y='Oscillations detected \n (per Model fit)', hue='method',
...
for knee in knee_list:
    exp = -2
    sig = sim_knee(n_seconds=n_secs, fs=fs,
                   exponent1=0, exponent2=exp, knee=knee ** np.abs(exp))
    f, psd = dsp.welch(sig, fs=fs, **kwargs_psd)
    df = pd.DataFrame({'Frequency (Hz)': f,
                        'Power': psd})
    df['knee'] = knee
...
sns.lineplot(df_1.query('`Frequency (Hz)` < 100'), legend=False,
             x='Frequency (Hz)', y='Power', hue='knee', palette=pal, ax=ax[0])
ax[0].set_xscale('log')
ax[0].set_yscale('log')

ax[1].scatter(df_ap.query('method == "pyrasa"')['Knee Frequency (Hz)'],
...
r, p = sts.pearsonr(df_ap.query('method == "pyrasa"')['Knee Frequency (Hz)'],
                    df_ap.query('method == "specparam"')['Knee Frequency (Hz)'],
)
r, p = np.round(r,2), np.round(p, 3)
...
for exp in exp_list:
    ...
    sig = sim_knee(n_seconds=n_secs, fs=fs,
                   exponent1=0, exponent2=exp, knee=15 ** np.abs(exp))
    f, psd = dsp.welch(sig, fs=fs, **kwargs_psd)
    df = pd.DataFrame({'Frequency (Hz)': f,
                        'Power': psd})
    df['exp'] = exp
...
sns.lineplot(df_1.query('`Frequency (Hz)` < 100'), legend=False,
             x='Frequency (Hz)', y='Power', hue='exp', palette=pal, ax=ax[0])
ax[0].set_xscale('log')
ax[0].set_yscale('log')

df_ap, df_p = get_simulations('exponent_knee_delta')

ax[1].scatter(df_ap.query('method == "pyrasa"')['Exponent_2'],
...
r, p = sts.pearsonr(df_ap.query('method == "pyrasa"')['Exponent_2'],
                    df_ap.query('method == "specparam"')['Exponent_2'],)
...
sns.lineplot(df_1.query('`Frequency (Hz)` < 50').query('`Frequency (Hz)` > 1'), legend=False,
             x='Frequency (Hz)', y='Power', hue='osc', palette=pal, ax=ax[0])
...
sns.barplot(df_p, #legend=False,
             x='method', y='Center Frequency Δ', hue='method', 
             palette='deep', hue_order=['specparam', 'pyrasa'], ax=ax[1])
ax[1].set_xlabel('')
...
sns.lineplot(df_1.query('`Frequency (Hz)` < 50').query('`Frequency (Hz)` > 2'), legend=False,
             x='Frequency (Hz)', y='Power', hue='bw', palette=pal, ax=ax[0])
...
sns.barplot(df_p, #legend=False,
             x='method', y='Bandwidth Δ', hue='method', 
...
sns.lineplot(df_1.query('`Frequency (Hz)` < 50').query('`Frequency (Hz)` > 2'), legend=False,
             x='Frequency (Hz)', y='Power', hue='height', palette=pal, ax=ax[0])
...
sns.lineplot(df_p, legend=False,
             x='sim_height', y='pw', hue='method', 
             palette='deep', ax=ax[1])
...
# %%
plt.scatter(df_p.query('method == "specparam"')['pw'],#*15,
            df_p.query('method == "specparam"')['sim_height']
            )


# %%
plt.scatter(df_p.query('method == "pyrasa"')['pw'],#*15,
            df_p.query('method == "pyrasa"')['sim_height']
            )

# %%
sts.pearsonr(df_p.query('method == "pyrasa"').dropna(axis=1)['pw'],#*15,
            df_p.query('method == "pyrasa"').dropna(axis=1)['sim_height'])


# %%
import scipy.signal as dsp
test_arr = np.array([1,1,1,2,3,4,5,4,3,2,1,1,])
dsp.find_peaks(test_arr, height=[0, 5], prominence=.5)
# %%
height = 1
freq = 10
exp = -1.
...
plt.loglog(freq, psd)
# %%
dsp.find_peaks(psd, height=[0, 5], prominence=.01, )
# %%
from pyrasa import irasa
from pyrasa.utils.peak_utils import get_peak_params

# %%
freq, ap, p = irasa(sig, fs=fs, 
                    band =(1,100), 
                    kwargs_psd={'nperseg':fs*2,
                                'noverlap':fs})
# %%
plt.plot(freq, p[0,:])
# %%
