...
start = time.time()
sgramm_aperiodic, sgramm_periodic, freqs_ap, time_ap = irasa_sprint(x_l,  fs=fs,
                                                       band=(1, 100),
                                                       hop=1,
                                                       freq_res=.5,
                                                       win_duration=2,
...
for ch in [0, 1]:
    ...
    axes[0].set_title('Combined \n spectrum (SPRINT)')
    axes[0].imshow(output['TF'][ch, :, freq_mask], 
            #extent=(tmin, tmax, new_freqs.min(), new_freqs.max()), 
            origin='lower',
            aspect='auto')
    ...
    axes[1].imshow(output['TF'][ch, :, freq_mask], 
            #extent=(tmin, tmax, new_freqs.min(), new_freqs.max()), 
            origin='lower',
            aspect='auto')
...
#%%
df_alpha_foof_cl = pd.DataFrame(fgs[0].get_params('peak_params'), columns=('CF', 'PW', 'BW', 'Time')).query('CF < 12').query('CF > 8')
