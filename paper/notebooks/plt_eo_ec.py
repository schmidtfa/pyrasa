...
swarm_kwargs = {'size': 10,
                'alpha': 0.5}
point_kwargs = {'markers': '_',
                'markersize': 20}
...
df_cmb = all_df_p.merge(all_df_ap, on=['s_id', 'condition', 'ch_name']).groupby(['s_id' , 'condition'])[['Offset', 'Exponent', 'cf', 'bw', 'pw']].mean().reset_index()
...
eo_im.plot_topomap(ch_type='eeg', times=0, 
                   colorbar=False, #vlim=(min, max),
                   cmap='Reds', axes=ax[0])

ec_im.plot_topomap(ch_type='eeg', times=0, 
                   colorbar=False, vlim=(min, max),
                   cmap='Reds', axes=ax[1])
# %%
