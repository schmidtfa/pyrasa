...
for f in all_files:
   ...
   cur_p = pd.concat([cur_f['specparam']['knee']['periodic'],
                      cur_f['pyrasa']['knee']['periodic']])#.dropna(axis=1)


   cur_ap = pd.concat([cur_f['specparam']['knee']['aperiodic'],
                          cur_f['pyrasa']['knee']['aperiodic']
                        ])#.dropna(axis=1)
...
df_ap_sim['sim_kf'] = df_ap_sim['sim_knee'] ** (1. / df_ap_sim['sim_exp'])
#%%
...
sns.barplot(df_ap_sim,#dropna().reset_index(), 
               x='method', 
               y='Knee Frequency Δ', 
               palette=palette,
               hue='method',
...
# %%
f, ax = plt.subplots(figsize=(4,4))
sns.barplot(df_ap_sim, x='method', 
               y='Exponent Δ', 
               hue='method',
               palette=palette,
               #split=True, 
...
sns.lineplot(df_ap_sim, x='sim_kf', 
               y='Exponent Δ', 
               hue='method',
               palette=palette,
               legend=False,
...
#%%
df_p_sim['sim_kf'] = df_p_sim['sim_knee'] ** (1. / df_p_sim['sim_exp'])
...
sns.lineplot(df_nona, x='sim_knee', 
               y='Center \n Frequency Δ', 
               hue='method',
               palette=palette,
               legend=False,
