...
def get_peak_params(periodic_spectrum,
    ...
    for ix, ch_name in enumerate(ch_names):
        ...
        cur_df = pd.DataFrame({'ch_name': 'chx',
                                'cf': freqs[peaks],
...
df_peaks = get_peak_params(periodic_spectrum=periodic_spectrum,
                ch_names=ch_names,
                freqs=freqs,
...
for row in cur_df.iterrows():
    ...
    axes[2].fill_between(freqs, row[1]['pw'], where=logical, facecolor='red', alpha=.5)
...
for row in cur_df.iterrows():
    ...
    ax.fill_between(freqs, row[1]['pw'], where=logical, facecolor='red', alpha=.5)
