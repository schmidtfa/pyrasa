...
def make_gao_ei(n_secs, fs, ei_ratio = 4):
    ...
    ampa = sim_synaptic_current(n_secs, fs=fs, n_neurons=8000, 
                                firing_rate=2, tau_r=0.0001, 
                                tau_d=0.002, t_ker=1) * (mv_rest - mv_rev_e)
    gaba = sim_synaptic_current(n_secs, fs=fs, n_neurons=2000, 
                                firing_rate=5, tau_r=0.0005, 
                                tau_d=0.01, t_ker=1) * (mv_rest - mv_rev_i)
...
df['Knee Frequency'] = df['knee'] ** (1. / df['exponent'])
df['tau'] = 1. / (2 * np.pi * df['Knee Frequency'])
