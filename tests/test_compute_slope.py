...
def test_slope_fitting_settings(
    ...
    with pytest.raises(AssertionError):
        compute_slope(psd[freq_logical], freqs[freq_logical], fit_func='fixed', fit_bounds=(0, 500))

    math_txt = 'The first frequency appears to be 0 this will result in slope fitting problems. '
    +'Frequencies will be evaluated starting from the next highest in Hz'
    with pytest.warns(UserWarning, match=math_txt):
