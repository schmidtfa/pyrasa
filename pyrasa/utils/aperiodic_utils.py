...
def _compute_slope(aperiodic_spectrum, freq, fit_func, fit_bounds=None, scale_factor=1):
    ...
    exp_guess = [np.abs(np.log10(aperiodic_spectrum[0] / aperiodic_spectrum[-1]) / np.log10(freq[-1] / freq[0]))] if fit_bounds == None else fit_bounds[1]
...
def compute_slope_sprint(aperiodic_spectrum,
                         freqs,
                         times,
                         fit_func, 
                         ch_names=[],
    ...
    for ix, t in enumerate(times):
        ...
        cur_aps, cur_gof = compute_slope(aperiodic_spectrum[:, :, ix], 
                               freqs=freqs,
                               fit_func=fit_func, 
                               ch_names=ch_names,
                               fit_bounds=fit_bounds
