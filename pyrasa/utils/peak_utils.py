...
def get_peak_params(
    ...
    """
    Extract peak parameters from the periodic spectrum extracted from IRASA.
    ...
    ch_names : Iterable, optional
        Channel names ordered according to the periodic spectrum. Defaults to None.
    smooth : bool, optional
        Whether to smooth the spectrum. Defaults to True.
    smoothing_window : int or float, optional
        Smoothing window in Hz handed over to the savitzky-golay filter. Defaults to 1.
    polyorder : int, optional
        Polynomial order for the savitzky-golay filter. Defaults to 1.
    cut_spectrum : tuple of float, optional
        Cut the periodic spectrum to limit peak finding to a sensible range. Defaults to None.
    peak_threshold : float, optional
        Relative threshold for detecting peaks. Defaults to 1.0.
    min_peak_height : float, optional
        Absolute threshold for identifying peaks. Defaults to 0.01.
    peak_width_limits : tuple of float, optional
        Limits on possible peak width, in Hz, as (lower_bound, upper_bound). Defaults to (0.5, 6.0).
    ...
    ------
    ValueError
        If the periodic spectrum contains NaN values.
    """
    ...
    for ix, ch_name in enumerate(ch_names):
        peaks, peak_dict = dsp.find_peaks(
            ...
            width=peak_width_limits / freq_step,
...
def get_peak_params_sprint(
    ...
    smooth : bool, optional
        Whether to smooth the spectrum. Defaults to True.
    smoothing_window : int or float, optional
        Smoothing window in Hz handed over to the savitzky-golay filter. Defaults to 2.
    polyorder : int, optional
        Polynomial order for the savitzky-golay filter. Defaults to 1.
    cut_spectrum : tuple of float, optional
        Cut the periodic spectrum to limit peak finding to a sensible range. Defaults to None.
    peak_threshold : int or float, optional
        Relative threshold for detecting peaks. Defaults to 1.
    min_peak_height : float, optional
        Absolute threshold for identifying peaks. Defaults to 0.01.
    peak_width_limits : tuple of float, optional
        Limits on possible peak width, in Hz, as (lower_bound, upper_bound). Defaults to (0.5, 12).
...
def get_band_info(df_peaks: pd.DataFrame, freq_range: tuple[int, int], ch_names: list) -> pd.DataFrame:
    """
    Extract peaks in a specified frequency range from the Peak DataFrame obtained via "get_peak_params".
