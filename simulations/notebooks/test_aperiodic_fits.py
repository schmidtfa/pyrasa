...
# %% test irasa
kwargs_psd = {'window': 'hann', 'nperseg': int(fs * duration), 'noverlap': int(fs * duration * overlap)}

# Calculate original spectrum
freq_ap, psds_ap = dsp.welch(knee_ap, fs=fs, **kwargs_psd)
fmax = 150
freq_rasa_ap, psd_aperiodics_ap, psd_periodics_ap = irasa(
    np.array(knee_ap), band=(0.1, fmax), fs=fs, kwargs_psd=kwargs_psd, hset_info=(1.0, 2.0, 0.01)
)
...
def mixed_model(x, x1, a1, b1, k, a2, b2, b3):
    ...
    condlist = [x < x1, x > x1]
    funclist = [lambda x: a1 - np.log10(x**b1), lambda x: a1 - np.log10((x - x1) ** b2 * (k + (x - x1) ** b3))]
...
curv_fit_kwargs['p0'] = np.array(
    knee_guess + off_guess + exp_guess + half_pw_off + knee_guess + exp_guess + exp_guess
)  # TODO: Really pay attention here
curv_fit_kwargs['bounds'] = (
...
freq_rasa, psd_aperiodics, psd_periodics = irasa(
    np.array(knee_osc), band=(0.1, 100), fs=fs, kwargs_psd=kwargs_psd, hset_info=(1.0, 2.0, 0.01)
)
...
get_peak_params(psd_periodics, freq_rasa, min_peak_height=0.01, peak_width_limits=(0.25, 8), peak_threshold=1)
